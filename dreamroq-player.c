    ...
    int texture_height, int colorspace)
{
    ...
    if (!graphics_initialized)
    {
        ...
        buf = (uint16*)memalign(32,512*512);
       // memset(textures[0],textures[1],512*512);
...
int main()
{
    ...
    pvr_init_defaults();
     pvr_dma_init(); 
   	...
   	snd_stream_init(0);
	mp3_init();
	printf("sndmp3_init();  called...\n");
	
	mp3_start("/rd/coe.mp3",0);
    thd_sleep(90);
    status = dreamroq_play("/cd/media/intro.roq", ROQ_RGB565, 0, &cbs);
	...
	pvr_mem_reset();
      ...
      int length;	
     //  iso_ioctl(0,NULL,0);
         length = fs_load("/cd/coe.bin", &subelf); // Reload mod menu from Quake menu with quit  Ian micheal 2019
