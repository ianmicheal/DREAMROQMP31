    ...
    int texture_height, int colorspace)
{
    ...
    /* send the video frame as a texture over to video RAM */
     pvr_txr_load_dma(buf, textures[current_frame], stride * texture_height * 2,1);
...
int main()
{
	...
	mp3_start("/cd/test.mp3",0);
    thd_sleep(10);
    status = dreamroq_play("/cd/bios.roq", ROQ_RGB565, 1, &cbs);
